"""
data_prep.py
Pipeline Step 1 — extracted from src/train.py → load_data()
"""
import os
import numpy as np
import pandas as pd
...
def main():
    ...
    csv_path = os.path.join(args.output_data, "loan_data.csv")
    df.to_csv(csv_path, index=False)

    print(f"    Saved raw data → {csv_path}")
    print(f"    Shape: {df.shape}  |  Classes: {df['default'].unique().tolist()}")
